...
def main():
    print("==============================================================")
    print("      *** RESUMING RENDERING ON ACTIVE INSTANCE ***  ")
    print("==============================================================")
    
    manager = VastManager(settings.vast_api_key)
    
    # 0. Automatically detect active instance
    instances = manager.list_instances()
    if not instances:
        print("[ERROR] No active instances found on your Vast.ai account!")
        sys.exit(1)
        
    active_inst = instances[0]
    instance_id = int(active_inst['id'])
    print(f"Successfully detected active instance: {instance_id}")
    sys.stdout.flush()
    
    # 1. Monitor Startup & SSH Connection of existing instance
    print("\n[1/5] Waiting for the active instance container to download and start SSH server...")
    ...
    for attempt in range(60):
        ...
        if not target_inst:
            print("[ERROR] Active instance not found. It may have been destroyed.")
            sys.exit(1)
            
        cur_state = target_inst.get('cur_state')
        status_msg = target_inst.get('status_msg')
        actual_status = target_inst.get('actual_status')
        print(f"      Attempt {attempt+1}: Status is '{cur_state}' | Actual: '{actual_status}' | Message: {status_msg}")
        sys.stdout.flush()
        ...
        if cur_state == "running" and target_inst.get('ssh_host') and actual_status == "running":
            ...
            print(f"\n      Container is ONLINE at {ssh_host}:{ssh_port}!")
            sys.stdout.flush()
            break
        time.sleep(15)
    ...
    # 2. Establish SSH Connection & Run Setup
    key_path = os.path.join(os.getcwd(), "vast_id")
    print(f"\n[2/5] Connecting via SSH to {ssh_host}:{ssh_port}...")
    sys.stdout.flush()
...
"""
    ...
    # 3. Inject all 6 scene prompts
    print(f"\n[3/5] Injecting {len(TEST_SCENES)} dynamic scenes into the ComfyUI queue...")
    sys.stdout.flush()
    ...
    # 4. Monitor progress until queue is empty
    print("\n[4/5] Rendering has started! Monitoring generation progress...")
    ...
    # 5. Download All Completed Videos
    print("\n[5/5] Commencing high-speed download of all rendered videos...")
    sys.stdout.flush()
    ...
    # 6. CLEAN UP - Destroy instance to preserve credit!
    print("\n==============================================================")
